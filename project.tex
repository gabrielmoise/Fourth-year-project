...
%The project will discuss methods to check this equivalence, together with results about decidability on the checking procedure.\newline
%michael: results are about expressiveness and decidability, not "checking" vs decidability 
The majority of computer science areas nowadays deal extensively with different forms of finite relational structures, usually with some form of constraints attached. In the databases domain, we can view the usual relations (tables) as structures, being constrained by dependencies, which impose relationships on the data that can appear in instances. In graph theory, we can view graphs as structures, and the constraints can specify properties of interest to us, for example k-coloring. \michael{It is not obvious how to view $k$-coloring as a TGD!}
In this project,
%we will try to formalize precisely the relational structures and constraints.
%michael: formalizing relational structures and constraints is not a contribution of the the these
...
%michael: connect constraints to schema mappings
%, examining their behavior with respect to database instances. 
%michael: This makes it sounds like their two goals of the thesis: examining behavior (very vague) and the deciding 
%equivalence. We agreed that the thesis is about equivalence, with results about expressiveness and then about decidability
...
\michael{Something very wrong here -- maybe a cut and paste error. What are "these relationships"? You 
need to describe the notion of relationship you are interested in, then say that logical constraints can describe them}
...
\michael{Maybe add "for GTGDs" to the subection title}
